...
const etudiantNote = async (req,res)=>{
    ...
    try {
        ...
        const notes = await Note.find({matriculeEtudiant:matetud})
        if(notes.length===0){
...
const insererNote = async (req,res)=>{
    ...
    try {
        if(!matetud || !francais || !mathematique || ! anglais || ! svt || !moyenneGenerale  ){
        ...
        const nouveau = new Note( {matetud,francais,mathematique,anglais,svt,moyenneGenerale})
...
// Mise a jour des notes
    const majNote = async (req,res)=>{
        const id = req.params.id
            const {matetud,francais,mathematique,anglais,svt,MG} = req.body
        try {
            const etudiant = await Etudiant.findOne({matricule:matetud})
            if(!etudiant){
                return res.status(404).json({message:"Aucun étudiant ne correspond a ce matricule"})
            }
            const majNotes = await Note.findByIdAndDelete(id, {matetud,francais,mathematique,anglais,svt,MG})
            return res.status(200).json({message:"Les notes ont été modifié avec succès !"})
        } catch (error) {
