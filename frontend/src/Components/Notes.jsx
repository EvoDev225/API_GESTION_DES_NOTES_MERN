...
const Notes = () => {
    ...
    const fetchEtudiant = async (id) => {
        try {
            const etudiants = await axios.get(`http://localhost:3000/classe/etudiant/${id}`)
            if (etudiants.data.Status === "Success") {
                setEtudiant(etudiants.data.data)
            }
        } catch (error) {
            console.log(error)
        }
    ...
    const insererNote = async (e) => {
        e.preventDefault();
        try {
            const note = await axios.post("http://localhost:3000/note", notes)
            ...
            toast.error("Une erreur est survenue lors de l'insertion des notes")
        }
    }
console.log(notes)
    return (
                ...
                <div className="w-full flex flex-col gap-8 my-15">
                    <div className="flex">
                        <div className="flex flex-col gap-4 px-20">
                            ...
                            <select onChange={e => fetchEtudiant(e.target.value)} className="px-8 py-3 outline-none border-gray-600 text-gray-500 border-3 rounded-xl w-50">
                                <option value="">Sélectionner</option>
                                {classe.map((c, index) => (
                                    <option key={index} value={c._id}>{c.nom}</option>
                                ))}
                            </select>
                        </div>
                        <div className="flex flex-col gap-4 px-20">
                            <h1 className="text-3xl font-bold">Nom & Prénoms</h1>
                            <select onChange={e => setNotes({ ...notes, matetud: e.target.value })}  className="px-8 py-3 outline-none border-gray-600 text-gray-500 border-3 rounded-xl w-50">
                                <option value="">Sélectionner</option>
                                {etudiant.map((e, index) => (
                                    <option key={index} value={e.matricule}>{e.nom} {e.prenom}</option>
                                ))}
                            </select>
                        </div>
                    </div>
                    <form onSubmit={insererNote} className="flex flex-col items-center gap-5">
                        {/* Français */}
